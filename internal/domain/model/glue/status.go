...
// TypeGlueStatus model info
// @Description Glue의 상태를 나타내는 구조체
type TypeGlueStatus struct {
...
var _glueStatus *TypeGlueStatus

func Status() *TypeGlueStatus {
	if _glueStatus == nil {
		lockGlueStatus.Do(
			func() {
				if gin.IsDebugging() {
...
func UpdateStatus() *TypeGlueStatus {
	if gin.Mode() == gin.ReleaseMode {
		...
		if err := json.Unmarshal(stdout, &_glueStatus); err != nil && gin.IsDebugging() {
		...
		if err := json.Unmarshal(stdout, &_glueStatus); err != nil {
			utils.FancyHandleError(err)
