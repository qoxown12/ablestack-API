...
func UpdateStatus() *TypePCSStatus {
	...
	if gin.Mode() == gin.ReleaseMode {
		...
		if err != nil {
			...
			if err := xml.Unmarshal(stdout, status); err != nil && gin.IsDebugging() {
				utils.FancyHandleError(err)
			}
		}
	} else {
		...
		if err := xml.Unmarshal(stdout, status); err != nil {
			utils.FancyHandleError(err)
...
func GetClone() []TypePCSClone {
	return Status().Resources.Clone
}

func GetResource() TypePCSResources {
	ret := []TypePCSResource{}

	ret = append(ret, Status().Resources.Resource...)
