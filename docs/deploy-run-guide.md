...
4. `/cube/deploy/run`으로 나머지 배포 단계를 실행한다. 이때 `scvm_prepare`/`ccvm_prepare` 뒤에 `scvm_bootstrap`/`ccvm_bootstrap`이 실행되어 VM API health 확인과 라이선스 후처리를 자동 수행한다.
5. 필요하면 `only:["scvm_bootstrap"]`, `only:["ccvm_bootstrap"]`으로 VM bootstrap 단계만 재실행한다.
...
| `scvm_bootstrap` | `scvm_prepare` 성공 후 자동 실행. `only`에 명시하면 단독 재실행 가능 | `bootstrap.scvm=true` |
...
| `ccvm_bootstrap` | `ccvm_prepare` 성공 후 자동 실행. `only`에 명시하면 단독 재실행 가능 | `bootstrap.ccvm=true` |
...
`scvm_bootstrap`/`ccvm_bootstrap`은 대상 VM의 `/api/v1/health`가 응답한 뒤 `/cube/license`로 라이선스를 등록하고 다시 status를 확인한다. `/api/v1/health`와 `/health`는 라이선스 등록 전에도 호출 가능한 public health check이다.
