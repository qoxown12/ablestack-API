...
CONFIG_PATH="${ABLESTACK_CEPH_SSH_CONFIG:-${ABLESTACK_CONFIG_PATH}/ceph_ssh_config}"
SSH_SCAN_SCRIPT="${ABLESTACK_SSH_SCAN_SCRIPT:-${ABLESTACK_CONFIG_PATH}/shell/ssh-scan.sh}"
if [[ ! -f "$SSH_SCAN_SCRIPT" && -f "${ABLESTACK_CONFIG_PATH}/shell/host/ssh-scan.sh" ]]; then
  SSH_SCAN_SCRIPT="${ABLESTACK_CONFIG_PATH}/shell/host/ssh-scan.sh"
fi
TARGET_HOST="scvm"
SSH_USER="root"
